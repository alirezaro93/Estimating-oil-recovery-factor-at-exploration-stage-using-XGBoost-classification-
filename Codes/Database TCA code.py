...
df = pd.read_csv("toris_commercial_atlas.csv", encoding = "ISO-8859-1",
                 engine = 'python')
...
plt.grid(False)
plt.savefig("Raw Histogram of RECOVERY FACTOR (OIL ULTIMATE)",
            bbox_inches='tight')
...
plt.grid(False)
plt.savefig('commercial_Atlas_TORISXGBC_average_impact_on_model_output_magnitude.png', bbox_inches='tight',facecolor='w',dpi=1200)
...
shap.summary_plot(shap_values[0], X_trainnum, feature_names = df_x.columns,
                  show = False)
plt.grid(False)
plt.savefig('commercial_Atlas_TORISXGBR_impact_on_model_output.png', bbox_inches='tight',facecolor='w')

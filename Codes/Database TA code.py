...
df = pd.read_csv("toris_atlas.csv", encoding = "ISO-8859-1",
                 engine = 'python')

dfcommercial = pd.read_csv("commercial.csv", encoding = "ISO-8859-1",
                 engine = 'python')

dfcommercial = dfcommercial.dropna(how='any')
...
commercial_accu = pd.DataFrame(columns = ['Train', 'accuracy'])

y_test_values = pd.DataFrame(columns = ['y_measured', 'y_estimated'])

y_train_values = pd.DataFrame(columns = ['y_measured', 'y_estimated'])

y_predcommercial_values = pd.DataFrame(columns = ['y_measured', 'y_estimated'])
...
joblib.dump(best_model,'best_model_toris_atlas_classified')
...
print('Feature Importance of the models fitted on the TORIS Atlas database')
...
plt.title('Database TA')
shap.summary_plot(shap_values, X_trainnum, plot_type = "bar", 
                  feature_names = df_x.columns, show = False)
plt.grid(False)
plt.savefig('TORIS_AtlasXGBC_average_impact_on_model_output_magnitude.png', bbox_inches='tight',facecolor='w', dpi=1200)
...
shap.summary_plot(shap_values[0], X_trainnum, feature_names = df_x.columns,
                  show = False)
plt.grid(False)
plt.savefig('TORIS_AtlasXGBR_impact_on_model_output.png', bbox_inches='tight',facecolor='w', dpi  =1200)


# for col in range(0, X_trainnum_columns, 1):
#     plt.figure(figsize=(10,6))
#     plt.title('TORIS Atlas ModelXGBR Dependency Plots')
#     shap.dependence_plot(col,shap_values, X_trainnum, feature_names=df_x.columns)
#     plt.grid(False)
#     plt.savefig('TORIS_AtlasXGBR_dependency_plot_{}.png'.format(df_x.columns([col])), bbox_inches='tight',facecolor='w')

#%%     

print("commercial data clean up.")

dfcommercial = dfcommercial[(dfcommercial['RECOVERY FACTOR'] >= 0) | (dfcommercial['RECOVERY FACTOR'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['RECOVERY FACTOR'] <= 1) | (dfcommercial['RECOVERY FACTOR'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['POROSITY'] >= 0) | (dfcommercial['POROSITY'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['POROSITY'] <= 1) | (dfcommercial['POROSITY'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['WATER SATURATION'] >= 0) | (dfcommercial['WATER SATURATION'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['WATER SATURATION'] <= 1) | (dfcommercial['WATER SATURATION'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['FVF'] >= 0) | (dfcommercial['FVF'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['FVF'] <= 10) | (dfcommercial['FVF'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['GOR'] >= 0) | (dfcommercial['GOR)'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['GOR'] <= 60) | (dfcommercial['GOR'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['RESERVES'] >= 0) | (dfcommercial['RESERVES'].isnull())]

dfcommercial = dfcommercial[(dfcommercial['RESERVES'] <= 5e+11) | (dfcommercial['RESERVES'].isnull())]
print('commercial data processing.')

Xcommercial = dfcommercial.iloc[:, :-1]

del Xcommercial["RECOVERY FACTOR (OIL ULTIMATE)"]

Xcommercial = gauss_scaler.fit_transform(Xcommercial)

Xcommercial = scaler.fit_transform(Xcommercial)

ycommercial = dfcommercial.iloc[:, -1]

encoder = OrdinalEncoder()

ycommercial = ycommercial.values.reshape(-1, 1)

ycommercial = encoder.fit_transform(ycommercial)

ycommercial = ycommercial.astype(int)

bestmodel = joblib.load('best_model_toris_atlas_classified')

y_predcommercial = bestmodel.predict(Xcommercial)

scorecommercial=accuracy_score(ycommercial, y_predcommercial)

print('')
print("commercial accuracy is equal to", scorecommercial)

macro_averaged_f1_commercial = f1_score(ycommercial, y_predcommercial, average = 'macro')
print(f"Macro-Averaged F1 score using sklearn library : {macro_averaged_f1_commercial}")

y_test=y_test.flatten()

y_train=y_train.flatten()

ycommercial=ycommercial.flatten()
...
commercial_score = commercial_accu.append({"Train": "Train", "accuracy_value" : scorecommercial},
                             ignore_index = True)
...
x_setcommercial = pd.DataFrame(data = Xcommercial, columns = df_x.columns)

y_predcommercial_values = pd.concat([y_predcommercial_values,pd.DataFrame({"y_measured": ycommercial,
                            "y_estimated": y_predcommercial})], ignore_index = True)
...
with pd.ExcelWriter('model_data.xlsx', engine="openpyxl") as writer:
    ...
    y_predcommercial_values.to_excel(writer, sheet_name = 'y_predcommercial_values')
            
    test_score.to_excel(writer, sheet_name = 'test_accuracy')
            
    train_score.to_excel(writer, sheet_name = 'train_accuracy')
    
    commercial_score.to_excel(writer, sheet_name = 'commercial_accuracy')
    ...
    x_setcommercial.to_excel(writer, sheet_name = 'x commercial')